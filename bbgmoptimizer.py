...
getPlayers("Minnesota", "FA", teamDict, 1980)
print("P----------------------------------")
i = 1
tp = 0
a = 0
...
for subset in itertools.combinations(playerList, 5):
    for item2 in subset:
        #print(item2)
        tp += item2.tp
        a += item2.A
    ...
    #print(str(dsyn))
  #  if set == 3:
    lineupList.append((osyn+dsyn+Rsyn, subset, osyn, dsyn, Rsyn))
    tp = 0
    a = 0
    B = 0
    po = 0
...
lineupList.sort(key=lambda x: x[0])
for item in lineupList[-15:]:
    for player in item[1]:
        print(player)
        f.write(str(player) + "\n")

    print("O: " + str(item[2]) + " D: " + str(item[3]) + " R: " + str(item[4]) )
    f.write("O: " + str(item[2]) + " D: " + str(item[3]) + " R: " + str(item[4]) + "\n")
    print("----------")
    f.write("----------"+ "\n")
f.close()
